...
export default function Header({ solidBg }: HeaderProps) {
	...
	return (
		<>
			<header
				className={cn(
					'absolute top-0 right-0 left-0 z-10 px-3 py-5 text-white xl:px-7',
				...
				<div className="dbs-container relative flex items-center md:py-5">
					...
					<Button variant="tertiary-dark" href="/about" className="hidden md:block md:text-lg" ariaLabel="About">
						About
					</Button>
					<a
						href="/"
						className="p-2 font-medium md:absolute md:top-0 md:left-1/2 md:-translate-x-1/2 md:transform md:p-5 md:text-xl xl:text-2xl">
						DATTA + BAUM
					</a>
					{/* <Button
            variant="tertiary-dark"
			...
			<Transition
				...
				<Dialog as="div" className="relative z-20" onClose={toggleMenu}>
					<Dialog.Panel className="fixed inset-0 flex flex-col bg-black px-4 py-6 text-3xl font-light text-white">
						<button className="mb-4 ml-auto self-start p-2 transition-colors hover:bg-neutral-950" onClick={toggleMenu}>
							<XIcon />
						</button>
						<a href="/watch" className="px-3 py-5 transition-colors hover:bg-neutral-950" onClick={toggleMenu}>
							Product/Watch
						</a>
						<a href="/about" className="px-3 py-5 transition-colors hover:bg-neutral-950" onClick={toggleMenu}>
							About
						</a>
						{/* <button
