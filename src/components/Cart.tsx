		...
		<div className="bg-gray-100 px-6 py-12 md:px-8 md:py-20 xl:px-12">
			<div className="flex flex-col">
					...
					<table className="w-full divide-y-2 divide-black border-b-2 border-b-black">
						...
						<tbody>
							{isLoading ? (
								...
								<tr>
									<td className="py-32 text-center text-base font-medium lg:py-28" colSpan={4}>
										Your cart is empty
									</td>
								</tr>
							) : (
								VARIANTS?.map((variant: any) => {
									...
									return (
										<tr className="flex flex-col align-top lg:table-row" key={variant.id}>
												...
												<div className="flex gap-4">
													...
													<div className="flex w-full flex-col gap-1">
														<div className="flex items-center justify-between gap-1">
															<h1 className="text-2xl font-light lg:text-4xl">{title}</h1>
															<h4 className="flex w-full justify-end text-xs font-medium md:text-base lg:hidden">
																{formatPrice(variant.price)}
															</h4>
														</div>
														<p className="ml-1 text-start text-xs font-light text-black md:text-sm">Color : {color}</p>
											...
											<td className="m-auto w-full max-w-[400px] pt-4 pb-8 align-top lg:w-fit lg:py-8">
												<div className="flex items-center justify-between gap-6 lg:justify-normal">
													<button
														className={`border border-gray-400 p-3 ${
															minusSignDisabled ? 'cursor-not-allowed opacity-60' : 'opacity-100'
														}`}
														onClick={() => updateSelectedVariants(variant.id, 'remove')}
														disabled={minusSignDisabled}>
														<MinusIcon className="h-4 w-4 text-black" />
													</button>
													<span className={'font-medium text-black'}>{variant.selectedQuantity}</span>
													<button
														className={`border border-gray-400 p-3 ${
															plusSignDisabled ? 'cursor-not-allowed opacity-60' : 'opacity-100'
														}`}
														onClick={() => updateSelectedVariants(variant.id, 'add')}
				...
				<div className="my-8 flex flex-col-reverse items-center justify-between gap-6 lg:flex-row-reverse">
					...
					<h4 className="text-base">Shipping will be calculated during checkout</h4>
				</div>
			</div>
		</div>
	);
