...
function useCart() {
  const [selectedVariants, setSelectedVariants] = useLocalStorage<any>(
    "CART_SELECTED_VARIANTS",
    "",
  );

  const { selectedTotalQuantity, selectedVariantsDetails } =
    React.useMemo(() => {
  ...
  const updateSelectedVariants = (
    ...
    else if (variantArr.indexOf(varId) !== -1)
      variantArr.splice(variantArr.indexOf(varId), 1);
